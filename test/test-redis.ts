...
function runTest(name: string, testFn: () => void): void {
    try {
        ...
        system.stdout.writeLine("  Error: " + ((e as Error).message || String(e)));
    }
    system.stdout.writeLine("");
}

// Test with Unix Domain Socket
runTest("Connect with Unix Domain Socket", () => {
    const redis = new Redis({socketPath:'/tmp/redis.sock'});
    if (!redis) {
...
runTest("Connect with 'password' parameter", () => {
    const redis = new Redis({socketPath:'/tmp/redis.sock', password:'test_password'});
    if (!redis) {
...
runTest("Connect with 'pw' parameter", () => {
    const redis = new Redis({socketPath:'/tmp/redis.sock', pw:'test_password'});
    if (!redis) {
...
runTest("Basic Redis operations", () => {
    const redis = new Redis({
        socketPath: '/tmp/redis.sock'
    });
